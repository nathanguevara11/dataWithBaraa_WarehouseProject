...
---------------------------------------------------------------------------------------
	COPY bronze.crm_customer_info(cst_id, cst_key, cst_firstname, cst_lastname, cst_marital_status, cst_gndr, cst_create_date) FROM 'C:\Users\librarypc\Documents\DWB_sql_project\datasets\source_crm\cust_info.csv' WITH (FORMAT csv, HEADER);  
	COPY bronze.crm_prd_info(prd_id, prd_key, prd_nm, prd_cost, prd_line, prd_start_dt, prd_end_dt) FROM 'C:\Users\librarypc\Documents\DWB_sql_project\datasets\source_crm\prd_info.csv' WITH (FORMAT csv, HEADER);
	COPY bronze.crm_sales_details(sls_ord_num, sls_prd_key, sls_cust_id, sls_order_dt, sls_ship_dt, sls_due_dt, sls_sales, sls_quantity, sls_price) FROM 'C:\Users\librarypc\Documents\DWB_sql_project\datasets\source_crm\sales_details.csv' WITH (FORMAT csv, HEADER);
	COPY bronze.erp_cust_AZ12 (CID, BDATE, GEN) FROM 'C:\Users\librarypc\Documents\DWB_sql_project\datasets\source_erp\CUST_AZ12.csv' WITH (FORMAT csv, HEADER);
	COPY bronze.erp_loc_A101(CID, CNTRY) FROM 'C:\Users\librarypc\Documents\DWB_sql_project\datasets\source_erp\LOC_A101.csv' WITH (FORMAT csv, HEADER);
	COPY bronze.erp_px_cat_g1v2 (ID, CAT, SUBCAT, MAINTENANCE) FROM 'C:\Users\librarypc\Documents\DWB_sql_project\datasets\source_erp\PX_CAT_G1V2.csv' WITH (FORMAT csv, HEADER);
